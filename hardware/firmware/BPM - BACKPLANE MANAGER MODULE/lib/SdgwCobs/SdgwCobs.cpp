...
bool SdgwCobs::encode(const uint8_t* in, size_t len, uint8_t* out, size_t outMax, size_t& outLen) {
    outLen = 0;
...
bool SdgwCobs::decode(const uint8_t* in, size_t len, uint8_t* out, size_t outMax, size_t& outLen) {
    outLen = 0;
    if (!out || (!in && len != 0)) return false;
