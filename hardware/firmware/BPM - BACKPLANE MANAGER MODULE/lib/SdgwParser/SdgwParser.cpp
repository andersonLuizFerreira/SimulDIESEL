...
bool SdgwParser::getFrame(Frame& out) const {
    if (!_hasFrame) return false;
    out = _last;
    return true;
}

SdgwParser::Result SdgwParser::push(uint8_t b) {
    _hasFrame = false;
    _hasHeader = false;

    if (b == (uint8_t)SDGW_COBS_DELIMITER) {
        if (_rxEncLen == 0) {
...
SdgwParser::Result SdgwParser::tryDecodeAndValidate() {
    _rxDecLen = 0;

    // 1) decode COBS
    if (!SdgwCobs::decode(_rxEnc, _rxEncLen, _rxDec, sizeof(_rxDec), _rxDecLen)) {
        _hasHeader = false;
        return FrameBadCobs;
    }
    ...
    if (_rxDecLen > (size_t)SDGW_MAX_LOGICAL_FRAME) return FrameTooLarge;

    // 4) valida CRC: último byte é CRC
    const uint8_t gotCrc = _rxDec[_rxDecLen - 1];
    const uint8_t calc = SdgwCrc8::compute(_rxDec, _rxDecLen - 1);
    ...
    if (dataLen > (size_t)SDGW_MAX_PAYLOAD) return FrameTooLarge;
